...
def main():
    ...
    while (time.time()*1000 - start_t) < 1000*60*5:
        increment = 256
        ahead = 2
...
def motion_calculator(manifest):
    ...
    if len(newList)>1:
        ...
        if present.sum() > 1 or prior.sum() > 1:
            ...
            #print(indexes)
            pairs = list(np.asarray(indexes[1:])-np.asarray(indexes[0:len(indexes)-1]))
            pairsPrior = list(np.asarray(indexesPrior[1:])-np.asarray(indexesPrior[0:len(indexesPrior)-1]))
            print(pairs)
