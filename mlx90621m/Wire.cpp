...
uint8_t TwoWire::transmitAndRequest(uint8_t quantity) 
{
  ...
  int8_t ret = twi_writeToReceiveFrom(txAddress, txBuffer, txBufferLength, rxBuffer, quantity);
...
uint8_t TwoWire::transmitAndRequest(int quantity) {
  return transmitAndRequest((uint8_t)quantity);
}
