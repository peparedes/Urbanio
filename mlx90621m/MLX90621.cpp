...
void MLX90621::setConfiguration() {
	...
	//byte defaultConfig_H = 0b00000100;
        byte defaultConfig_H = 0b01000110;
	Wire.beginTransmission(0x60);
	Wire.send(0x03);
	Wire.send((byte) Hz_LSB - 0x55); //Hace el complemento a 2
	Wire.send(Hz_LSB);
...
void MLX90621::readEEPROM() {
	...
	Wire.send(0x00);
        //Wire.transmitAndRequest(256);	
        Wire.requestFrom(0x50, 256);
	for (int i = 0; i <= 255; i++) {
		eepromData[i] = Wire.receive();
...
void MLX90621::readEEPROM() { // Read in blocks of 32 bytes to accomodate Wire library
  for(int j=0;j<256;j+=32) {
    Wire.beginTransmission(0x50);
    Wire.send(j);
    byte rc = Wire.transmitAndRequest(32);
    if(rc) {
...
void MLX90621::calculateTO() {
	...
	for (int i = 0; i < 64; i++) {
		...
		alpha_ij[i] += (eepromData[0x80 + i]
				/ pow(2, eepromData[CAL_DELTA_A_SCALE]));
		alpha_ij[i] /= pow(2, 3 - resolution);
		v_ir_norm = v_ir_tgc_comp / (alpha_ij[i] - tgc * alpha_cp);
		v_ir_comp = v_ir_norm / emissivity;
		temperatures[i] = sqrt(sqrt((v_ir_comp + pow((Tambient + 273.15), 4))))
				- 273.15;
	}
}
/*
void MLX90621::readIR() {
	...
	//Wire.endTransmission(I2C_NOSTOP);
        Wire.transmitAndRequest(128);
	//Wire.requestFrom(0x60, 128);
	for (int i = 0; i < 64; i++) {
		byte pixelDataLow = Wire.receive();
		byte pixelDataHigh = Wire.receive();
...
void MLX90621::readPTAT() {
	...
	Wire.send(0x01);
	//Wire.endTransmission(I2C_NOSTOP);
        Wire.transmitAndRequest(2);
	//Wire.requestFrom(0x60, 2);
	byte ptatLow = Wire.receive();
	byte ptatHigh = Wire.receive();
	Wire.endTransmission();
...
void MLX90621::readCPIX() {
	...
	//Wire.requestFrom(0x60, 2);
        Wire.transmitAndRequest(2);
	byte cpixLow = Wire.receive();
	byte cpixHigh = Wire.receive();
	Wire.endTransmission();
...
uint16_t MLX90621::readConfig() {
	...
	//Wire.requestFrom(0x60, 2);
        Wire.transmitAndRequest(2);
	byte configLow = Wire.receive();
	byte configHigh = Wire.receive();
	Wire.endTransmission();
